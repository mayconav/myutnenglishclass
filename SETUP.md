...
Desde la pestaña **🎯 Bingo** del panel de administrador, el profesor abre una sala
para **un solo grupo a la vez** (el que tenga enfrente en ese momento). Solo los
alumnos con ese `grupo` asignado pueden entrar; al hacerlo, cada uno crea su avatar
(emoji + color + apodo) antes de recibir su cartón. El profesor controla el cantador
(reproducir/pausar, velocidad, cantar siguiente) y ve en tiempo real quién se ha unido
y quién completó una línea primero.

- **Una sola sala activa a la vez**: se guarda en el documento único
  `bingoSessions/current` de Firestore. Abrir una sala nueva para otro grupo borra
  automáticamente a los jugadores de la sala anterior.
- **El cantador corre en el navegador del profesor**: mientras la sala esté en
  "Reproducir", su pestaña debe permanecer abierta para que los verbos se sigan
  cantando automáticamente.
- Las reglas de seguridad ya actualizadas en `firestore.rules` (sección
  `bingoSessions`) restringen quién puede controlar la sala (solo el profesor, por
  correo) y quién puede escribir su propio avatar/cartón (solo cada alumno el suyo).
  Recuerda volver a publicar `firestore.rules` después de sustituir
  `PROFESOR_EMAIL_AQUI` (paso 4) para que esta sección también tenga el correo real.
